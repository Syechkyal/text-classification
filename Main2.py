...
class Ui_Setting(object):
    ...
    def setting_ui(self, MainWindow):
        ...
        self.groupbox_praproses.setGeometry(QtCore.QRect(10, 90, 301, 31))
        ...
        self.actionbutton_prapengolahan.setGeometry(QtCore.QRect(10, 50, 131, 23))
        ...
        self.Label_HasilPrapengolahan.setGeometry(QtCore.QRect(50, 90, 301, 23))
        font = QtGui.QFont()
        ...
        self.seleksifiturIG = QtWidgets.QRadioButton(self.groupbox_metode)
        self.seleksifiturIG.setGeometry(QtCore.QRect(130, 10, 131, 17))
        self.seleksifiturIG.setObjectName("seleksifiturIG")
        self.seleksifiturCS = QtWidgets.QRadioButton(self.groupbox_metode)
        self.seleksifiturCS.setGeometry(QtCore.QRect(230, 10, 141, 17))
        self.seleksifiturCS.setObjectName("seleksifiturCS")
        self.seleksifiturMI = QtWidgets.QRadioButton(self.groupbox_metode)
        self.seleksifiturMI.setGeometry(QtCore.QRect(330, 10, 151, 17))
        ...
        self.radioButtonnilaic_3.setGeometry(QtCore.QRect(220, 0, 82, 17))
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Sistem Klasifikasi Pertanyaan Berbahasa Indonesia"))
        self.actionbutton_prapengolahan.setText(_translate("MainWindow", "Input Data Awal"))
        self.Label_HasilPrapengolahan.setText(_translate("MainWindow", "  Tampilan Hasil Data Praproses"))
        ...
        self.seleksifiturIG.setText(_translate("MainWindow", "Seleksi Fitur IG"))
        self.seleksifiturCS.setText(_translate("MainWindow", "Seleksi Fitur CS"))
        ...
        self.Label_JumlahFitur_2.setText(_translate("MainWindow", "Pengujian Pertanyaan"))
        self.Label_JumlahFitur_3.setText(_translate("MainWindow", "Masukkan Pertanyaan :"))
        self.actionbutton_klasifikasi_test.setText(_translate("MainWindow", "Lakukan Pengujian"))
        self.Label_JumlahFitur_4.setText(_translate("MainWindow", "Hasil Klasifikasi           :"))
   ...
   #method aktivitas pengguna ketika ingin membuat model dan melatihnya/ Klasifikasi
    def tombolklasifikasi(self):
        ...
        if msg.exec_()==QtWidgets.QMessageBox.Yes:
            ...
            elif self.ui.seleksifiturIG.isChecked():
                if self.get_kernel() != -1 and self.get_c() != -1 and self.get_k()!= 0:
                    kernel_clf = self.get_kernel()
                    c_clf = self.get_c()
                    k_fs = self.get_k()
                    dataset = self.model.feature_selectionIG(k_fs)
                    dataset.to_excel('Asset/datasethasilIG.xlsx',index=False)
                    #dataset.to_excel('assets/dataset_seleksiIG.xlsx',index=False)
                    score = self.model.classify(dataset,kernel_clf,c_clf)
            ...
            elif self.ui.seleksifiturCS.isChecked():
                if self.get_kernel() != -1 and self.get_c() != -1 and self.get_k()!= 0:
                    kernel_clf = self.get_kernel()
                    c_clf = self.get_c()
                    k_fs = self.get_k()
                    dataset = self.model.feature_selectionCS(k_fs)
                    dataset.to_excel('Asset/datasethasilCS.xlsx',index=False)
                    #dataset.to_excel('assets/dataset_seleksiCS.xlsx',index=False)
                    score = self.model.classify(dataset,kernel_clf,c_clf)
                    score = np.array(score)
                    data_model = TabelView(score)
                    self.ui.tampil_hasil_train.setModel(data_model)
    ...
    def open_file(self):
        ...
        if fileName:
            ...
            pertanyaan = dataset.pertanyaan.tolist()
            #label = dataset.label.tolist()
            data_view = []
            for i in range(1,100):
                temp = []
                temp.append(pertanyaan[i])
                #temp.append(label[i])
    ...
    def testing(self):
         ...
         if self.ui.tanpaseleksifitur.isChecked():
               ...
               text = self.ui.masukan_komentar.text()
               prediction = self.model.predictseleksifiturIG(text)
               self.ui.tampil_hasil_komentar.setText(prediction)
         elif self.ui.seleksifiturCS.isChecked():
               text = self.ui.masukan_komentar.text()
               prediction = self.model.predictseleksifiturCS(text)
               self.ui.tampil_hasil_komentar.setText(prediction)
         elif self.ui.seleksifiturMI.isChecked():
               text = self.ui.masukan_komentar.text()
